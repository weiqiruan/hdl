...
localparam STATE_DO_RESET = 'h0;
localparam STATE_RESET = 'h1;
localparam STATE_DISABLED = 'h2;
localparam STATE_STARTUP = 'h3;
localparam STATE_ENABLED = 'h4;
localparam STATE_SHUTDOWN = 'h5;
 ...
 * domains to clear all residue state. The reset pulse is long enough so that it
...
wire [5:0] reset_chain;
wire [4:0] reset_chain_clks = {clk, src_clk, dest_clk, src_clk, clk};
...
assign reset_chain[0] = 1'b0;

generate
genvar i;

for (i = 0; i < 5; i = i + 1) begin: reset_gen
  ...
  if (GEN_ASYNC_RESET[i] == 1'b1) begin
    ...
    always @(posedge reset_chain_clks[i] or posedge do_reset) begin
      if (do_reset == 1'b1) begin
        ...
        reset_shift <= {reset_chain[i], reset_shift[3:1]};
      end
    end

    assign reset_chain[i+1] = reset_shift[0];

  end else begin
    assign reset_chain[i+1] = reset_chain[i];
  end
end
...
assign dest_resetn = ~reset_chain[3];
assign src_resetn = ~reset_chain[4];
assign req_resetn = ~reset_chain[5];
